...
const AdminReports = () => {
  ...
  const handleExportPDF = async () => {
    try {
      ...
      message.success(language === 'ar' ? 'تم تحميل ملف PDF بنجاح - استخدم Print to PDF في المتصفح' : 'PDF downloaded successfully - Use Print to PDF in browser')
    } catch (err) {
      console.error('Export PDF error:', err)
      message.error(language === 'ar' ? 'فشل تحميل PDF' : 'Failed to download PDF')
    }
  }
  ...
  const handleExportCSV = async () => {
    try {
      ...
      message.success(language === 'ar' ? 'تم تحميل ملف CSV بنجاح' : 'CSV downloaded successfully')
    } catch (err) {
      console.error('Export CSV error:', err)
      message.error(language === 'ar' ? 'فشل تحميل CSV' : 'Failed to download CSV')
    }
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'revenue',
      key: 'revenue',
      render: (revenue) => `${revenue || 0} ${language === 'ar' ? 'ريال' : 'SAR'}`,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 relative z-10">
        <div>
          ...
          <p className="text-base sm:text-lg text-gray-600 font-medium">
            {language === 'ar' ? 'تحميل وتصدير جميع التقارير' : 'Download and export all reports'}
          </p>
        </div>
        <Space wrap>
            <RangePicker
          ...
          <Col xs={24} sm={12} md={6}>
            <Card className="modern-kpi-card card-hover shadow-professional-lg border-0">
              <Statistic
                ...
                suffix="SAR"
                valueStyle={{ color: '#7a8c66', fontSize: '24px', fontWeight: 'bold' }}
              />
            </Card>
          </Col>
          <Col xs={24} sm={12} md={6}>
        ...
        <div className="overflow-x-auto">
          <Table
            ...
            pagination={{ 
              ...
              showTotal: (total) => {
                return language === 'ar' ? `إجمالي ${total} تقرير` : `Total ${total} reports`;
              },
