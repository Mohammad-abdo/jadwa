...
const AdminArticleEditor = () => {
  ...
  const fetchArticle = async () => {
    try {
      ...
      message.error(
        language === "ar" ? "فشل تحميل المقال" : "Failed to load article"
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleImageUpload = async (file, isFeatured = false) => {
    try {
      ...
      const response = await fetch(
        "https://jadwa.developteam.site/api/files/upload",
        {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      console.log("Saving article with data:", {
        ...articleData,
        content: finalContentEn.substring(0, 50) + "...",
      });
      ...
      message.error(
        err.message ||
          (language === "ar" ? "فشل حفظ المقال" : "Failed to save article")
      );
    } finally {
      setSaving(false);
    }
        ...
        <Row gutter={24}>
          <Col xs={24} lg={16}>
            <Card className="mb-6">
              ...
              <Divider>
                {language === "ar" ? "محرر المحتوى" : "Content Editor"}
              </Divider>
          ...
          <Col xs={24} lg={8}>
            ...
            <Card
              title={language === "ar" ? "الصورة الرئيسية" : "Featured Image"}
              className="mb-6"
            >
              {featuredImage ? (
          ...
          <Space>
            ...
            <Button
              ...
              onClick={() => {
                ...
                message.info(
                  language === "ar" ? "معاينة المقال" : "Preview article"
                );
              }}
              size="large"
            >
