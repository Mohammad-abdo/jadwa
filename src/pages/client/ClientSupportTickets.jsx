...
import { Card, Table, Button, Modal, Form, Input, Select, Tag, Space, Empty, message, Descriptions, Timeline } from 'antd'
import { PlusOutlined, EyeOutlined, MessageOutlined, CheckCircleOutlined } from '@ant-design/icons'
...
const ClientSupportTickets = () => {
  ...
  const handleCreateTicket = async (values) => {
    try {
      ...
      message.success(language === 'ar' ? 'تم إنشاء التذكرة بنجاح' : 'Ticket created successfully')
      setCreateModalVisible(false)
      form.resetFields()
      fetchTickets()
    } catch (err) {
      message.error(language === 'ar' ? 'فشل إنشاء التذكرة' : 'Failed to create ticket')
    }
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 relative z-10">
        <div>
          ...
          <p className="text-base sm:text-lg text-gray-600 font-medium">
            {language === 'ar' ? 'إنشاء ومتابعة تذاكر الدعم الفني' : 'Create and track support tickets'}
          </p>
        </div>
        <Button
          type="primary"
      ...
      >
        ...
        >
          <Form.Item
            ...
            rules={[{ required: true, message: language === 'ar' ? 'الرجاء إدخال الموضوع' : 'Please enter subject' }]}
          >
            <Input />
          </Form.Item>
          ...
          <Form.Item
            ...
            rules={[{ required: true, message: language === 'ar' ? 'الرجاء إدخال الوصف' : 'Please enter description' }]}
          >
            <TextArea rows={6} />
          </Form.Item>
